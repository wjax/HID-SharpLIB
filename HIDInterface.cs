...
namespace HIDSharpLIB
{
    public class HIDDevice
    {
        ...
        private void initDevice(string devicePath, bool useAsyncReads)
        {
            ...
            productInfo.manufacturer = manfString;
            productInfo.product = productName;
            int.TryParse(SN, out productInfo.serialNumber);
            productInfo.PID = (ushort)attributes.ProductID;
            productInfo.VID = (ushort)attributes.VendorID;
            productInfo.versionNumber = (ushort)attributes.VersionNumber;
            productInfo.IN_reportByteLength = (int)capabilities.InputReportByteLength;
            ...
            FS_read = new FileStream(handle_read, FileAccess.ReadWrite, capabilities.OutputReportByteLength > 0 ? capabilities.OutputReportByteLength : 1, false);
            FS_write = new FileStream(handle_write, FileAccess.ReadWrite, capabilities.InputReportByteLength > 0 ? capabilities.InputReportByteLength : 1, false);
        ...
        public int read(byte[] readBuf)
        {
            ...
            //Call readFile
            int read = FS_read.Read(readBuf, 0, readBuf.Length);
            return read;
        }
        #endregion
